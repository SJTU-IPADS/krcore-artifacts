...
int volatile tri_cnt = 0;
int volatile use_rc = 0;
...
main(int argc, char** argv)
{
    ...
    for(int i =0 ; i < FLAGS_run_sec; ++i) {
        sleep(1);
        if (triggered && (++tri_cnt) >= 3) use_rc = 1;
    }
...
usize worker(const usize qd, const usize& worker_id, Statics* s)
{
    Statics& ss = *s;
    r2::util::FastRandom r(0xdeafbeaf + worker_id);
    // running elastic node
    const int pop_cnt = 2048;
    core_req_t req_list[pop_cnt];
    int mod = 1024 * 1024 * 4 / FLAGS_payload_sz;
    for (int i = 0; i < pop_cnt; ++i) {
        int offset = r.next() % mod;
        req_list[i] = { .addr = 0,
                .length = FLAGS_payload_sz,
                .lkey = 32,
                .remote_addr = offset * FLAGS_payload_sz,
                .rkey = 32,
                .send_flags = i == 0 ? 1 : 0,
                .vid = 0,
                .type = Read
    ...
    while (running) {
        ...
        for (int i = 0; i < 3; ++i) {
            for(int j = 0 ; j < FLAGS_or_sz; ++j) {
                int offset = r.next() % mod;
                req_list[0].remote_addr = offset * FLAGS_payload_sz;
                req_list[0].send_flags = j ==0 ? 1 : 0;
                req_list[0].type = i == 3 ? Write : Read;
                assert(qpush(qd, &req) == 1);
            }
            while (qpop(qd, &reply) != 1) {}
...
main_worker_fn(const usize& worker_id, Statics* s)
{
    ...
    if (triggered) {
        ...
        */
        ...
        subworker.push_back(
                new Thread_t(std::bind(worker, qd, worker_id, s)));
