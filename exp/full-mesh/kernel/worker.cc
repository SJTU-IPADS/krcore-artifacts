  ...
  */
  ...
  while (running) {
    ...
    for (RecvIter<RC, entry_cnt> iter(recv_cq, wcs); iter.has_msgs();
      ...
      // Do connect work
      {
        req_list[0].type = Write;
        req_list[0].send_flags = 1;
        for (int i = 0; i < (FLAGS_worker_num - 1); ++i) {
            int conn_res = qconnect(kv_qd, gid_str, strlen(gid_str), (worker_id % 24));
        }
        req_list[0].type = SendImm;
      }
