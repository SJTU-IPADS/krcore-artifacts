...
impl<'a> Parser<'a> {
    ...
    fn read_guid_addr_impl(&mut self) -> Option<Guid> { 
        let mut bs : [u16;8] = [0;8];
        let mut i = 0;
        while i < 8 {
            if i != 0 && self.read_given_char(':').is_none() {
                return None;
    ...
    fn read_guid_addr(&mut self) -> Option<Guid> {
        self.read_atomically(|p| p.read_guid_addr_impl())
    }
...
impl FromStr for IpAddr {
    ...
    fn from_str(s: &str) -> Result<IpAddr, AddrParseError> {
        if let Some(addr) = Parser::new(s).read_till_eof(|p| p.read_ipv4_addr()) {
            Ok(IpAddr::V4(addr))
        } else if let Some(addr) = Parser::new(s).read_till_eof(|p| p.read_ipv6_addr()) {
            Ok(IpAddr::V6(addr))
        } else {
            Err(AddrParseError(()))
        }
...
impl FromStr for Guid {
    type Err = AddrParseError;
    fn from_str(s: &str) -> Result<Guid, AddrParseError> {
        match Parser::new(s).read_till_eof(|p| p.read_guid_addr()) {
            Some(s) => Ok(s),
            None => Err(AddrParseError(())),
        }
    }
