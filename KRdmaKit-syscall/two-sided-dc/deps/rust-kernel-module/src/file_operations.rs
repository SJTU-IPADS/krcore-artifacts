...
unsafe extern "C" fn ioctrl_callback<T: FileOperations>(
    //_inode: *mut bindings::inode,
    file: *mut bindings::file,
    cmd: c_types::c_uint,
    arg: c_types::c_ulong,
) -> c_types::c_long {
    let f = &mut *((*file).private_data as *mut T);
    f.ioctrl(cmd, arg)
}
...
pub struct FileOperationsVtable<T>(marker::PhantomData<T>);

impl<T: FileOperations> FileOperationsVtable<T> {
    pub const VTABLE: bindings::file_operations = bindings::file_operations {
        open: Some(open_callback::<T>),
        release: Some(release_callback::<T>),
        unlocked_ioctl: Some(ioctrl_callback::<T>),
...
pub trait FileOperations: Sync + Sized {
    ...
    fn open(file: *mut bindings::file) -> KernelResult<Self>;

    fn ioctrl(&mut self, cmd: c_types::c_uint, arg: c_types::c_ulong) -> c_types::c_long;

    fn mmap(&mut self, vma: *mut bindings::vm_area_struct) -> c_types::c_int;
