 ...
 */
static inline int
qconnect(int qd,  const char *addr, int addr_sz,int port, int vid = 0) {
    req_connect_t req;
    reply_t reply;
    reply.status = err;
...
static inline int
qpush(int qd, const push_core_req_t* reqq, uint8_t pop_res = 0, uint32_t push_recv_cnt = 0)
{
    push_req_t req;
    reply_t reply;
    req.req.reply_buf = &reply;
...
static inline int
qpush_recv(int qd, uint32_t push_count)
{
    push_recv_req_t req;
    reply_t reply;
...
// pop an RDMA request from the queue
static inline int
qpop_msgs(int qd, pop_reply_t * reply, uint32_t pop_count, uint32_t payload_sz = 8)
{
    pop_msgs_t req;
