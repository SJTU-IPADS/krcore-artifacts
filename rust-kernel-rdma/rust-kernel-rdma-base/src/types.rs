...
impl core::fmt::Debug for sa_path_rec { 
    fn fmt(&self, fmt: &mut ::core::fmt::Formatter) -> core::fmt::Result {
        let s_gid = Guid::new_u8(unsafe {&self.sgid.raw}); 
        let d_gid = Guid::new_u8(unsafe {&self.dgid.raw}); 

        write!(fmt, "sa_path_res: src {} -> dst {}, service id: {}", s_gid, d_gid, self.service_id)
    }    
}
