...
pub fn assert_dmesg_ends(msgs: &[&[u8]]) {
    let output = Command::new("dmesg").output().unwrap();
    assert!(output.status.success());
    let lines = output.stdout.split(|x| *x == b'\n').collect::<Vec<_>>();
    let mut lines: &[&[u8]] = &lines;
    for msg in msgs {
    //        let pos = lines.iter().position(|l| String::from_utf8((*l).to_vec()).contains(&String::from_utf8(msg.to_vec())));
...
pub fn assert_dmesg_contains(msgs: &[String]) {
    let output = Command::new("dmesg").output().unwrap();
    assert!(output.status.success());
    ...
    for msg in msgs { 
        let pos = lines.position(|l| l.contains(msg));
        assert!(pos.is_some());
    }
}

pub fn dmesg_contains(msg: &String) -> bool {
    let output = Command::new("dmesg").output().unwrap();
    assert!(output.status.success());
    let stdout = String::from_utf8(output.stdout).unwrap();

    let mut lines = stdout.lines();
    lines.position(|l| l.contains(msg)).is_some()
}
