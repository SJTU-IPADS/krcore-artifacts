...
impl IBExplorer {
    ...
    #[inline]
    pub fn resolve_v2(&mut self, service_id: u64,
                      ctx: &RContext,
                      dst_gid: &String) -> KernelResult<&mut Self> {
        let dst_gid = crate::net_util::str_to_gid(dst_gid);
        let mut sa_client: ib_sa_client = unsafe { core::mem::MaybeUninit::zeroed().assume_init() };
        unsafe { ib_sa_register_client(crate::to_ptr!(sa_client)) };

        let ret = self.resolve_inner(service_id, ctx, dst_gid, &mut sa_client as _);

        unsafe { ib_sa_unregister_client(crate::to_ptr!(sa_client)) };
...
pub unsafe extern "C" fn path_rec_completion(
    ...
    e.path_result = if status != 0 {
        // !("Failed to query the path with error {}", status);
        None
    } else {
        Some(*resp)
    };
