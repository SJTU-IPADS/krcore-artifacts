...
impl<'a> RCOp<'a> {
    ...
    /// Pop with param. In this way, the caller has the responsibility to
    /// check whether wc.status is valid (valid if equal to 0), and free the wc afterwards
    #[inline]
    pub fn pop_with_wc(&mut self, wc: *mut ib_wc) -> Option<*mut ib_wc> {
        let ret = unsafe { bd_ib_poll_cq(self.qp.get_cq(), 1, wc) };
        if ret < 0 {
            return None;
        } else if ret == 1 {
            return Some(wc);
