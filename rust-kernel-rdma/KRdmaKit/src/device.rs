...
impl RNIC {
    ...
    pub fn open(&self) -> Option<RContext> {
        RContext::create(self)
    }
...
impl<'a> RContext<'a> {
    pub fn create(hca: &'a RNIC) -> Option<Self> {
        //1. allocate the PD
        let pd = unsafe {
            ib_alloc_pd(
                hca.get_raw_device(),
    ...
    pub fn get_hca(&self) -> &RNIC {
        self.hca
    }

    pub fn explore_path(&self, gid: String, service_id: u64) -> Option<sa_path_rec> {
        let mut exp = crate::ib_path_explorer::IBExplorer::new();
        let res = exp.resolve_v2(service_id, self, &gid); 
        match res {
            Ok(_) => exp.get_path_result(), 
            Err(_) => None
        }
    }
    ...
    pub fn get_subnet_prefix(&self) -> u64 {
        unsafe { self.hca.default_gid.global.subnet_prefix as u64 }
    }

    /// fetch interface id; used for UD interactions
    #[inline]
    pub fn get_interface_id(&self) -> u64 {
        unsafe { self.hca.default_gid.global.interface_id as u64 }
    }
