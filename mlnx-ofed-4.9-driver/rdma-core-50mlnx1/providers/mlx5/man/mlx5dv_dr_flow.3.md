...
*mlx5dv_dr_action_create_dest_vport* creates a forwarding action to a **vport** on the same **domain**. Valid only on domain type FDB. mlx5dv_dr_action_create_dest_ib_port is similar to dest_vport action but instead of vport number the input is the ib_port.
