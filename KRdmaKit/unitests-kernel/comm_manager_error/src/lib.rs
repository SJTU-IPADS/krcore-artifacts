...
impl CMCallbacker for CMHandler {
    ...
    fn handle_sidr_req(
        ...
        log::info!("cm in server side: 0x{:X}", reply_cm.raw_cm_id().as_ptr() as u64);
...
impl CMCallbacker for CMHandlerClient {
    ...
    fn handle_sidr_rep(
        ...
        log::info!(
            "client sidr response received, event {:?}, private data: {:?}, rep_param {:?}",
            event.event,
            event.private_data,
            rep_param
        );

        log::info!("client side cm: 0x{:X}", reply_cm.raw_cm_id().as_ptr() as u64);

        if rep_param.status != ib_cm_sidr_status::IB_SIDR_SUCCESS { 
            log::error!("failed to send SIDR {:?}", rep_param);
        }
        log::info!("client accidentally returns an error in sidr rep handler");
        Err(CMError::CallbackError(0))
    }
}
...
fn test_cm() {
    ...
    let _send_res = client_cm.send_sidr(req, 73 as usize);
    let res = c_handler_ref.0.wait(500);

    if res.is_ok() {
        log::error!("linux completion should timeout here");
    } else {
