...
impl PreparedQueuePair {
    ...
    //     };
    //
    //     let _ = then_send_async(&mut write, req).await.map_err(|_| {
    //         log::error!("IO error");
    //         ControlpathError::CreationError(
    ...
    pub fn handshake_sync(self, addr: SocketAddr) -> Result<Arc<QueuePair>, ControlpathError> {
        // check qp type
        // must be rc and never initialized
        if self.inner.mode != QPType::RC {
            log::error!("bring up type check error!");
