...
use alloc::sync::Arc;
use core::pin::Pin;
use crate::cm::{ServerCM, EndPoint};
...
use crate::consts::{DEFAULT_RPC_HINT, MAX_KMALLOC_SZ};
...
pub struct RCtrl<'a> {
    ...
    pub registered_rcs: HashMap<usize, Arc<RC>>,
    ...
    recv_buffer: Option<Arc<RecvHelper<N>>>,
    /// CM for connections
    cm: ServerCM,
...
impl<'a> RCtrl<'a> {
    ...
    pub fn create(id: u64, ctx: &'a RContext<'a>) -> Option<Pin<Box<Self>>> {
        let res = Self {
            id,
            context: ctx,
            cm: Default::default(),
        ...
        ServerCM::new(
            ...
            boxed.cm = cm;
            boxed.cm.listen(id).unwrap();    // listen to service_id = id
            boxed
        })
    }
}
...
impl<'a> RCtrl<'a> {
    pub unsafe extern "C" fn cm_handler(cm_id: *mut ib_cm_id, env: *const ib_cm_event) -> c_types::c_int {
        ...
        match event.event {
            ib_cm_event_type::IB_CM_REQ_RECEIVED => {
                ...
                let _ = ctrl.rc_conn_callback(cm_id, event);
    ...
    pub fn rc_conn_callback(&'a mut self, cm: *mut ib_cm_id, event: ib_cm_event) -> KernelResult<()> {
        ...
        match qp.bring_to_rtr(cm) {
            Ok(_) => {
                ...
                #[cfg(feature = "profile")]
                    {
                        ...
                        self.profile.report(3);
                        self.profile = Profile::new();
                    }

                let mut qp_duplicate = false;
                self.mutex.lock();
                {
                    let new_key = new_key as usize;
                    if !self.registered_rcs.contains_key(&new_key) {
                        self.registered_rcs.insert(new_key as usize, in_qp_clone);
                        reply.status = Status::Ok;
                    } else {
                        qp_duplicate = true;
                        reply.status = Status::Exist;
                    }
                }
                self.mutex.unlock();
                ...
                if qp_duplicate {
                    // this rc qp will be dropped after the function returns
                    // we should not destroy the cm inside the rc here
                ...
                // push recv
                {
                    ...
                    let _ = recv.post_recvs(ib_qp, null_mut(), 512);
                }
    ...
    // Vid is used in two-sided communication
    #[inline]
    pub fn get_next_in_qp_key(&self, vid: u64) -> u64 {
        if vid == 0 {
...
impl<'a> RCtrl<'a> {
    ...
    #[inline]
    pub fn get_rc(&'a self, qd: usize) -> Option<&'a Arc<RC>> {
        self.registered_rcs.get(&qd)
    }

    #[inline]
    pub fn get_endpoint(&'a self) -> EndPoint {
        let ctx = self.get_context();
        let ud = self.get_ud(DEFAULT_RPC_HINT).unwrap();
        EndPoint {
            qpn: ud.get_qp_num(),
            qkey: ud.get_qkey(),
            lid: ctx.get_lid(),
    ...
    #[inline]
    pub fn get_self_dct(&self) -> *mut ib_dct { self.dct_server.get_inner_dct() }

    #[inline]
    pub fn get_recv_buffer(&'a mut self) -> &'a mut Arc<RecvHelper<N>> {
